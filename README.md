...
**210913 추가** users.resolvers photo 내림차순
...
`nodemon` 프로젝트를 진행할 때 도와주는 모듈이기 때문에 빌드과정에서는 빼준다.

배포 시 메모리용량이 커지기 때문에 `babel-node`를 사용하면 안됨 -> `babel CLI` 사용

1. `src`폴더를 만들어주고 프로젝트 관련 파일을 모두 넣어준다.
2. 환경이 바뀌었으므로 `npm run dev` 명령어를 수행하기위해 `package.json`에서 `src/server`로 변경
3. `"build"` 커맨드 추가, `babel src --out-dir build`란 "babel, src폴더에 있는 모든걸 가져와서 'build'라는 폴더를 만들어서 거기에 다 담아줘"라는 것임. 이후 `$ npm run build`
4. build 폴더가 생겼으니 `.gitignore`에 `/build/`추가
5. `"start"` 커맨드 추가, `$ npm run start` 혹은 `$ npm start`.
   이대로 진행하면 `var _isLiked = _asyncToGenerator( /*#__PURE__*/regeneratorRuntime.mark(function _callee(_ref8, _, _ref9)...ReferenceError: regeneratorRuntime is not defined...`라는 에러가 뜸.
6. 에러가 뜬 이유는 비동기함수 때문, 에러를 해결하기 위해 📌 `npm install --save-dev @babel/plugin-transform-runtime`
7. `babel.config.json`에 plugin 추가.
8. build 폴더를 지워주고 다시 `npm run build`실행 후 `npm start`
9. 4000포트(graphql서버)로 가서 잘 되나 확인해본다.
